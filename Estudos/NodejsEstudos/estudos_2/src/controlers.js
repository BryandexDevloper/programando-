...
const recuperar_senha = async (req,res)=>{
 try{
  ...
  const ress = await dataBase.query('INSERT INTO verification_codes (userId, code, type, expiresAt) VALUES (?, ?, ?, ?)',[data.id,codigo,'recuperar_senha',new Date(Date.now() + 15*60*1000)])
  ...
  return res.status(200).json({mensagem:'Codigo de redifinição de senha enviado '})
 }catch(err){
...
const validar_senha = async (req,res)=>{
  try{
    ...
    if(ress.length === 0){
      return res.status(400).json({mensagem:'Codigo de veficacão invalido ou expirado'})
    }

    // Compara o código enviado com o código armazenado no banco
    if(data.codigo != usuario.code){
      return  res.status(400).json({mensagem:'Codigo de verificação invalido'})
    }

    // Verifica se o código ainda está dentro do prazo de validade (15 minutos)
    if(new Date(Date.now()) > usuario.expiresAt){
      return res.status(400).json({mensagem:'Codigo de verificação expirado'})
    }
    ...
    const mudarSenha = await dataBase.query('UPDATE users SET passwordHash = ? WHERE id = ?',[senhaHash,data.id])
    ...
    enviarEmail({email:data.email,html:html,subject:'Alteração de senha concluida',text:'Alteração de senha'})
    
    // Remove o código de verificação usado do banco de dados
    const apagar_codigo = dataBase.query('DELETE FROM verification_codes WHERE userId = ?',[data.id])

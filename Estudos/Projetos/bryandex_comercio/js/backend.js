...
servidor.put('/alterarperfil', (req, res) => {
    const data = req.body

    if (data.nome) {
        try {
            conexao.query(sql_mudar_nome, [validator.whitelist(String(data.nome), 'a-zA-Z0-9@_.-'), validator.whitelist(String(data.email), 'a-zA-Z0-9@_.-')], (err) => {
                if (err) {
                    return res.status(400).json({ mensagem: 'Desculpe ocorreu um erro inesperado' })
                }
            })

            res.status(200).json({ mensagem: 'Nome alterado com sucesso', sucesso: true })

        } catch (err) {
            res.status(400).json({ mensagem: 'Ocorreu um erro inesperado', erro: err })
        }
    ...
    if (data.telefone) {
        ...
        let codigo = Math.floor(Math.random() * 145751)
        const {email} = req.body
        conexao.query(sql_validar_cadastro, [codigo], (err) => {
            if (err) {
                ...
                try {
                    ...
                    const data = await fetch('https://api-envio-email-bryandex.onrender.com/enviar_mail', {
                        ...
                        body: JSON.stringify({
                            ...
                            to: email,
                            subject: 'Codigo de verificação para criação de conta',
                            html: htmlEmail, // aqui vem a nova tela de alterção de numero
                            email_user: email
                        })
                    })
                    ...
                    if(!resultado){
                        ...
                        return res.status(200).json({mensagem:resultado.mensagem,sucesso:true})
                    }
